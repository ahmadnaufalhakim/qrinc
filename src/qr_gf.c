...
static const uint8_t qr_gf_exp_table[255] = {
...
static const uint8_t qr_gf_log_table[256] = {
...
uint8_t qr_gf_exp(uint8_t x) {
    return qr_gf_exp_table[x];
}

uint8_t qr_gf_log(uint8_t x) {
    return qr_gf_log_table[x];
}
...
uint8_t qr_gf_mul(uint8_t x, uint8_t y) {
    ...
    return qr_gf_exp((uint8_t)(((uint16_t)qr_gf_log(x) + (uint16_t)qr_gf_log(y)) % 255));
}
